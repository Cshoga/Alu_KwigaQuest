...
def lessons():
    data = request.json
    l = Lesson(title=data.get('title'), content=data.get('content'), author_id=data.get('author_id'))
    db.session.add(l)
    db.session.commit()
...
def progress(user_id):
    if request.method == 'GET':
        ...
        return jsonify({'lesson_ids': p.lesson_ids,
...
def add_class():
    data = request.json
    t = AuthService.create_user(
        data.get('teacher_username'),
        data.get('teacher_password'),
        role='teacher',
